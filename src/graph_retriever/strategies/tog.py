...
@register_strategy("tog")
class ToGStrategy(BaseTraversalStrategy):
    ...
    def get_name(self) -> str:
        return f"ToG(iter={self.max_iterations}, beam={self.beam_width})"
    ...
    def traverse(
        ...
        **kwargs,
        ...
        if not valid_seeds:
            return TraversalResult(metadata={"strategy": "tog", "seeds_found": 0})

        # beam: 每條 path 是一個 list of entity_name
        beam: List[List[str]] = [
            [s["entity_name"]] for s in valid_seeds[:self.beam_width]
        ]

        all_visited_nodes: set = set()
        all_visited_edges: set = set()

        for s in valid_seeds:
            all_visited_nodes.add(s["entity_name"])

        actual_iterations = 0

        for iteration in range(self.max_iterations):
            actual_iterations = iteration + 1
            logger.info("ToG 迭代 %d/%d, beam size=%d", iteration + 1, self.max_iterations, len(beam))

            candidate_paths: List[tuple] = []  # (path, score)

            for path in beam:
                tail = path[-1]
                neighbors = self._get_neighbors(nx_graph, tail)

                for nb, edge_data in neighbors:
                    if nb in set(path):
                        continue
                    new_path = path + [nb]
                    score = self._score_path(
                        nx_graph, new_path, edge_data, query
                    )
                    candidate_paths.append((new_path, score))
            ...
            if not candidate_paths:
                logger.info("ToG 迭代 %d: 無新候選 path，提前結束", iteration + 1)
                break

            candidate_paths.sort(key=lambda x: x[1], reverse=True)
            beam = [p for p, _ in candidate_paths[:self.beam_width]]

            if self._check_sufficiency(nx_graph, beam, query):
                logger.info("ToG 迭代 %d: 資訊充分，提前結束", iteration + 1)
                break
    ...
    def _llm_score_path(
        ...
        try:
            ...
            logger.warning("ToG LLM scoring 失敗: %s", e)
            return 5.0
    ...
    def _check_sufficiency(
        ...
        try:
            ...
            logger.warning("ToG sufficiency check 失敗: %s", e)
            return False
