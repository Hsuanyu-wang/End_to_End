...
# This script extracts unique contexts from a given directory of JSONL files.
# The unique contexts are saved to a JSON file.
# The script is used to prepare the data for the RAG system.
# The script is used to prepare the data for the RAG system.
########################################################################
...
def extract_unique_contexts(input_directory, output_directory):
    ...
    for file_path in jsonl_files:
            ...
            with open(file_path, "r", encoding="utf-8") as infile:
                for line_number, line in enumerate(infile, start=1):
                    ...
                    try:
                        ...
                        context = json_obj.get("text_resource").get("text")
                        metadata = json_obj.get("metadata")
                        id = metadata.get("NO")
                        if context and context not in unique_contexts_dict:
                            unique_contexts_dict[context] = None
                        else:
                            print(f"context already exists: {id}")
        ...
        try:
            ...
            print(f"Unique `text` entries have been saved to: {output_filename}")
        except Exception as e:
