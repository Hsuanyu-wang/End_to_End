...
QUERY_MODES = ["naive", "local", "global", "hybrid", "mix", "custom"] 
...
async def main():
    ...
    with open(docs_path, "r", encoding="utf-8") as f:
        for line in f:
            if line.strip():
                ...
                docs_list.append(json.dumps(doc_json, ensure_ascii=False))
    
    # 5. 插入文本
    # [修改點 2] 移除 os.listdir() 的粗糙檢查，直接呼叫 ainsert。
    # LightRAG 內部會自動比對已建構的 Cache，跳過完成的部分，只補全未完成的 Graph
    print("正在處理文本並建立/補全索引，已處理過的內容會自動跳過...")
    await rag.ainsert(docs_list)
    ...
    for current_mode in QUERY_MODES:
        ...
        if HAS_RAGAS:
            ...
            eval_llm = ChatOllama(model="qwen2.5:14b", base_url=OLLAMA_URL, timeout=300)
